...
const Partita = ({ partita, resetAll, occhioApertoPartita, setOcchioApertoPartita }) => {
	...
	const isPartitaInCoppiaRegSelected = coppiaRegSelected.some((coppia) => coppia.team1 === partita.team1 && coppia.team2 === partita.team2);
		...
		if (numeroPartita !== 0 && numeroPartita === partita.numero) {
			...
			if (!isKQBtnActive) {
				...
				setSqSelected((currentSelected) => {
					...
					updatedSelection = updatedSelection.filter((squadra) => squadra !== partita.team1 && squadra !== partita.team1 + 'X' && squadra !== partita.team1 + 'Y' && squadra !== partita.team1 + 'Z' && squadra !== partita.team2 && squadra !== partita.team2 + 'X' && squadra !== partita.team2 + 'Y' && squadra !== partita.team2 + 'Z');
					if (selectionType === 'X') {
			...
			if (currentSelected) {
				return currentSelected.filter(
					(squadra) =>
						...
						squadra !== partita.team1 && squadra !== partita.team1 + 'X' && squadra !== partita.team1 + 'Y' && squadra !== partita.team1 + 'Z' && squadra !== partita.team2 && squadra !== partita.team2 + 'X' && squadra !== partita.team2 + 'Y' && squadra !== partita.team2 + 'Z'
				);
			}
		});
	};
	...
	return (
		<>
			<div
				className={`text-cyan-600/70  font-bold flex items-center justify-center sm:mx-[1rem] 
			...
			>
					...
					{!isPartitaInCoppiaRegSelected && (
						<div className={`{flex sm:pl-0 hover:cursor-context-menu z-10 }`}>
							<div className={`{sm:pr-1 pl-0 ${!isPartitaInCoppiaRegSelected} ? ml-[-0.5rem] mr-[-2rem] : 'pl-4'}`}>
								<span role="img" aria-label="" onClick={() => handleResetColors()}>
									〰️
								</span>
							</div>
						</div>
				...
				<div className="relative flex flex-col sm:ml-[2rem] justify-start w-[90rem] max-w-[70%] sm:mx-2">
					<div className="relative flex flex-row items-center ml-[1rem] xs:text-xs sm:text-xl">
						...
						<div
							className={`absolute flex flex-row ml-[40%] border border-sky-900 rounded-lg bg-gray-950 mt-3 w-6 pt-[0.7rem] z-[4] 
				...
				<button className="bg-black text-white font-bold z-[10] hover:cursor-pointer" onClick={() => toggleEye()} data-partita-numero={partita.numero}>
					{/* 🗨️ */}
					{isEyeOpen ? '👁️' : '🗨️'}
				</button>
			</div>
