			...
			<table>
				<thead>
					<tr className="bg-black text-white border border-black overflow-x-hidden text-md">
						...
						<th className="text-center text-cyan-700 w-[18%] sm:w-[25%] xl:w-[20%] font-bold">C</th>
						...
						{datiSquadra.map((partita, index) => {
							...
							return (
								<tr key={index} className={`overflow-x-hidden xs:text-lg sm:text-md ${bgHoverClass} last-text ${borderStyle}`}>
									<td className={`w-[5%] sm:w-[15%] xl:w-[5%] text-center font-bold text-cyan-500 bg-black text-xl`}>
										{isPronostico ? (
											<div className="flex justify-center items-center">
												<span className="text-yellow-400 text-xl font-b lack justify-item-centre ">*</span>
											</div>
										) : (
											<>
									...
									<td className={`w-[7%] sm:w-[15%] xl:w-[10%] text-center xs:text-xl sm:text-base text-black font-bold ${casaClass}`}>{partita.casa}</td>
									<td className={`w-[7%] sm:w-[15%] xl:w-[10%] text-center xs:text-xl sm:text-base text-black font-bold ${fuoriClass}`}>{partita.fuori}</td>
									<td className={`sm:w-[50%] pl-4 text-xl ${sqVsClass} `}>{isMobile == true ? sqVsFormatted.slice(0, 3) : sqVsFormatted}</td>
								</tr>
							);
						})}
					</tbody>
