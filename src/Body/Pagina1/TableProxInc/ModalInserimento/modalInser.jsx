    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
            <div className="bg-white p-5 rounded relative w-[30%] max-w-[100rem]">
                <button
                    onClick={isClose}
                    className="absolute top-0 right-0 m-2 text-red-500 text-5xl font-bold"
                    style={{ outline: 'none' }}>
                    &#10005; {/* Carattere speciale per la croce */}
                </button>
                <div className="mt-[10%] flex border border-8 rounded-2xl border-blue-500 mx-4"> {/* Contenitore Flex */}
                        ...
                        <div className="flex-1">
                            ...
                            {Array.from({ length: 10 }).map((_, index) => (
                                <form key={index} className="mb-2">
                                    <input
                                        type="text"
                                        placeholder={`Sq C ${index + 1}`}
                                        className={`w-full p-1 border border-gray-300 rounded font-bold text-2xl placeholder-gray-300 ${inputColors[index]}`}
                ...
                <div className="flex justify-center mt-4">
                    <button type="submit" className="px-4 py-2 bg-blue-500 text-white rounded text-2xl font-extrabold" onClick={handleSubmit}>INVIA</button>
                </div>
                {errorMessage && (
