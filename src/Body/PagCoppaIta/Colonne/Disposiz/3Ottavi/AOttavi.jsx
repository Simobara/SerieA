...
const AOttavi = ({ width = '70px', height = '24px' }) => {
	...
	const teamStyle = "bg-red-900 text-white filter brightness-75 pl-1 text-xl font-bold overflow-hidden";
	...
	const createMatch = (match, positionStyle) => {
		...
		return (
			<div key={match.id} className="absolute flex flex-col space-y-1" style={positionStyle}>
				<div className="flex">
					<div className={`${boxStyle} ${teamStyle}`} style={containerStyle}>{match.team1}</div>
